...
const OUT = join(__dirname, "..", "src", "data", "squads", "teams");

const HEADER = `import type { ClubSquadTemplate } from "@/data/squads/types";
...
function fmtSquad(slug, players) {
  const lines = [`  squad("${slug}", [`];
  for (const [name, pos] of players) {
    lines.push(`    { name: "${name.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}", position: "${pos}" },`);
  }
  lines.push("  ]),");
  return lines.join("\n");
}

function writeFile(filename, exportName, squads) {
  let body = HEADER + `\nexport const ${exportName}: ClubSquadTemplate[] = [\n`;
  body += squads.map(([s, p]) => fmtSquad(s, p)).join("\n");
  body += "\n];\n";
  writeFileSync(join(OUT, filename), body, "utf8");
  console.log(`Wrote ${filename} (${squads.length} clubs)`);
  return body;
}
...
["Glen Kamara","MF",29],["Crysencio Summerville","MF",23],["Wilfried Gnonto","FW",21],["Joel Piroe","FW",25],
["Georginio Rutter","FW",22],["Lukas Nmecha","FW",25],["Crysencio Summerville","FW",23],
]],
["ASTONVIL",[
...
["Stephy Mavididi","FW",26],["Tom Cannon","FW",23],["Facundo Buonanotte","FW",20],
]],
["NOTTINGH",[
...
["Chris Wood","FW",33],["Danilo","FW",23],["Elliot Anderson","FW",22],
]],
["COVENTRY",[
...
["Freddie Ladapo","FW",31],["George Hirst","FW",26],["Marcus Harness","FW",28],
]],
["SUNDERLA",[
...
["Kevin Schade","FW",23],["Ivan Toney","FW",28],["Frank Onyeka","FW",27],
]],
["CRYSTAL",[
...
["Odsonne Édouard","FW",26],["Adam Wharton","FW",20],["Daichi Kamada","FW",28],
]],
["QPR",[
...
["Evanilson","FW",25],["Samu Omorodion","FW",20],["Pepê","FW",27],["Samu Aghehowa","FW",22],
]],
["SPORTING",[
...
["Bradley Barcola","FW",22],["Gonçalo Ramos","FW",23],["Désiré Doué","FW",19],
]],
["MARSELHA",[
...
["Neal Maupay","FW",28],["Duncan McGuire","FW",23],["Valentin Rongier","FW",29],["Amine Gouiri","FW",24],
]],
["MONACO",[
...
["Mika Biereth","FW",21],["Maghnes Akliouche","FW",22],["George Ilenikhena","FW",18],["Myron Boadu","FW",23],
]],
["LYON",[
...
["Malick Fofana","FW",19],["Jefferson Silva","FW",22],["Ernest Nuamah","FW",21],["Gift Orban","FW",22],
]],
["LILLE",[
...
["RENNES",[
["Gustavo Hamel","GK",27],["Lorenz Assignon","GK",24],
["Lorenz Assignon","DF",24],["Arthur Theate","DF",24],["Alidu Seidu","DF",24],["Guéla Doué","DF",22],
["Baptiste Santamaria","DF",29],["Martin Terrier","DF",27],["Hans Hateboer","DF",30],
["Baptiste Santamaria","MF",29],["Martin Terrier","MF",27],["Ludovic Blas","MF",26],["Jota Silva","MF",25],
["Amine Gouiri","MF",24],["Glen Kamara","MF",29],["Albert Grønbæk","MF",23],["Amine Gouiri","FW",24],
["Jota Silva","FW",25],["Ludovic Blas","FW",26],["Martin Terrier","FW",27],["Albert Grønbæk","FW",23],
]],
];

function fixDupes(squads, repsMap) {
  return squads.map(([slug, players]) => {
    const reps = repsMap[slug] ?? [];
    const seen = new Set();
    const out = [];
    let repI = 0;
    for (const p of players) {
      if (seen.has(p[0])) {
        while (repI < reps.length && seen.has(reps[repI][0])) repI++;
        if (repI < reps.length) {
          out.push(reps[repI++]);
          seen.add(out[out.length - 1][0]);
        }
        continue;
      }
      seen.add(p[0]);
    ...
    while (out.length < 20 && repI < reps.length) {
      if (!seen.has(reps[repI][0])) {
        out.push(reps[repI]);
        seen.add(reps[repI][0]);
      }
      repI++;
    }
    return [slug, out.slice(0, 21)];
  });
}

const EN_FIXED = fixDupes(EN, {
  LEEDS: [["Daniel James","FW",27]],
  SOUTHAMP: [["Adam Armstrong","FW",27]],
  COVENTRY: [["Kasey Palmer","FW",28]],
  IPSWICH: [["Conor Chaplin","FW",27]],
  ...
  CRYSTAL: [["Will Hughes","FW",29]],
  QPR: [["Jimmy Dunne","FW",27]],
  SHEFFIEL: [["Sander Berge","FW",26]],
  WIMBLEDO: [["Ryan Heneghan","MF",24],["Nik Tzimas","FW",22]],
});

const PT_FIXED = fixDupes(PT, {
  FC_PORTO: [["Galeno","FW",27]],
  BRAGA: [["Al Musrati","FW",28],["Sikou Niakaté","MF",25]],
});

const FR_FIXED = fixDupes(FR, {
  PSG: [["Lucas Beraldo","DF",21]],
  MARSELHA: [["Valentin Rongier","MF",29]],
  MONACO: [["Denis Zakaria","FW",28]],
  LYON: [["Corentin Tolisso","FW",30],["Ernest Nuamah","MF",21]],
  LILLE: [["Ismaily","MF",34],["Ayyoub Bouaddi","FW",17]],
  LENS: [["Kevin Danso","MF",26],["Deiver Machado","MF",31],["Florian Sotoca","FW",34]],
  RENNES: [["Glen Kamara","FW",29],["Hans Hateboer","MF",30]],
});

for (const [label, squads] of [["EN", EN_FIXED], ["PT", PT_FIXED], ["FR", FR_FIXED]]) {
  for (const [slug, players] of squads) {
    const names = players.map((p) => p[0]);
    const gk = players.filter((p) => p[1] === "GK").length;
    const dupes = names.filter((n, i) => names.indexOf(n) !== i);
    if (players.length < 18 || players.length > 22) console.warn(`${label}/${slug}: ${players.length} players`);
    if (gk < 2) console.warn(`${label}/${slug}: ${gk} GK`);
    if (dupes.length) console.warn(`${label}/${slug}: dupes ${[...new Set(dupes)]}`);
  }
}

const enBody = writeFile("en.ts", "EN_SQUADS", EN_FIXED);
const ptBody = writeFile("pt.ts", "PT_SQUADS", PT_FIXED);
const frBody = writeFile("fr.ts", "FR_SQUADS", FR_FIXED);

console.log("\n===FILE: src/data/squads/teams/en.ts===");
console.log(enBody);
console.log("===FILE: src/data/squads/teams/pt.ts===");
console.log(ptBody);
console.log("===FILE: src/data/squads/teams/fr.ts===");
console.log(frBody);
